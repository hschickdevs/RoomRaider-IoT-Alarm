...
void sendPingRequest() {
  if (checkWifiConnection()) {
    ...
    StaticJsonDocument<100> doc; // Smaller size as payload is simpler
    doc["action"] = "ping";
    ...
    if(httpResponseCode > 0) {
      ...
      M5.Lcd.println("Ping Error: " + String(httpResponseCode));
    }
...
void loop() {
  ...
  // Deprecate conditional posts in favor of constant updates for connect/disconnect tracking:
  ...
  // Send the ping request each period to keep track of connect/disconnect
  // sendPingRequest();
